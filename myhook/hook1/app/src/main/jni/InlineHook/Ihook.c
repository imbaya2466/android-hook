...
bool HookArm(INLINE_HOOK_INFO* pstInlineHook)
{
    ...
    while(1)
    {
        ...
        if(BuildOldFunction(pstInlineHook) == false) //构建旧的代码模块并修复
        {
 ...
 * @return               原地跳转指令是否构造成功
 */
bool UnRebuildHookTarget(INLINE_HOOK_INFO* pstInlineHook)
{
    bool bRet = false;
    ...
    while(1)
    {
        ...
        LOGI("LIVE4.3");
        //恢复指令
        memcpy(pstInlineHook->pHookAddr, pstInlineHook->szbyBackupOpcodes, pstInlineHook->backUpLength);
...
{
    UnRebuildHookTarget(pstInlineHook);
}