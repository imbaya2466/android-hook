...
void * GetModuleBaseAddr(pid_t pid, char* pszModuleName)
{
    ...
    while (fgets(szFileLineBuffer, sizeof(szFileLineBuffer), pFileMaps) != NULL)
    {
        //LOGI("%s\n",szFileLineBuffer);
        //LOGI("%s\n",pszModuleName);
        if (strstr(szFileLineBuffer, pszModuleName)) //返回首次出现的位置，没有子串为null
        {
            LOGI("%s\n",szFileLineBuffer);
            char *pszModuleAddress = strtok(szFileLineBuffer, "-");//返回分割出的第一个
            if (pszModuleAddress)
            {
                ulBaseValue = strtoul(pszModuleAddress, NULL, 16);//16进制，化为unsigned long
...
bool BuildArmJumpCode(void *pCurAddress , void *pJumpAddress)
{
    ...
    while(1)
    {
        ...
        //PC-4的原因是三级流水。实际PC在当前地址的+8处，只有直接使用PC的指令要小心这个问题
        //LDR PC, [PC, #-4]
        //addr
        ...
        cacheflush(*((uint32_t*)pCurAddress), 8, 0); //刷新内存内容，因为有缓存机制
